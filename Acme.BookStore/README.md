...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Acme.BookStore.Blazor` and `Acme.BookStore.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 38091c92-6c07-4e81-a000-c39bfcd437ee
```

> `38091c92-6c07-4e81-a000-c39bfcd437ee` is the password of the certificate, you can change it to any password you want.
